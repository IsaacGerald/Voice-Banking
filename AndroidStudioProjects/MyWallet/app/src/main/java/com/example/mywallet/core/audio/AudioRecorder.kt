package com.example.mywallet.core.audio
...
class AudioRecorder(val context: Context) {
    ...
    fun startRecording(_fileName: String? = null, _internalStorage: Boolean = false) {
        val fileName = _fileName ?: "Recording_${System.currentTimeMillis()}.wav"
        audioFileName = fileName
        val path = if (_internalStorage) context.filesDir?.path + "/$fileName" else
        ...
        recorder = AudioRecord(
            ...
            RECORDER_SAMPLE_RATE, RECORDER_CHANNELS, RECORDER_AUDIO_ENCODING, 512
        )
